...
//		System.out.println(birthday);
		 ...
		 * resultsBean
		 */
		//TODO javaクラスで求めた本日の日付を利用して検索するように変更する（ResultsDaoの変更もあり）
		ResultsBean rb = ResultsDao.selectByBirthday(birthday);
		// //resultsBean.toString()をSysoutする
		...
		if (rb.getOmikuji_id() != null) {
			...
			//TODO 画面表示用に形を整える↓
			//TODO 画面出力のためのメソッドにする（最後で良い）
			showDisplay(ob);
//			System.out.println("確認用のomikuji_id:"+ob.getOmikuji_id());
//			System.out.println(ob.getFortune_name());
//			System.out.println(ob.getWish());
//			System.out.println(ob.getBusiness());
//			System.out.println(ob.getStudy());

		} else if (rb.getOmikuji_id() == null) {
			/**
			 * ⑤resultsテーブルに入力した結果がない場合→おみくじを生成する
			 * １、omikujiテーブルのレコード数(sqlのcountを使用)でrandomの数字を取得
			 ...
			 */
//			System.out.println("新しくひきます");

			Random random = new Random();
			//TODO DBの接続して、randomの引数をSQLのCountを使用して取得する
			int randomValue = random.nextInt(48);
			String omikuji_id = Integer.toString(randomValue + 1);
			//TODO　取得したomikuji_id・誕生日・今日の日付をresultsテーブルにINSERTする

			//TODO omikuji_idを条件にomikujiテーブルからデータを取得して、コンソールに出力する
			System.out.println(omikuji_id);
